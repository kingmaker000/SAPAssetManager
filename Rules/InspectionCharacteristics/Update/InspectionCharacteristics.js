    ...
    */
        ...
        try {
            if (context.binding.Formula1) {
                ...
                if (!foundError) {
                    ...
                    return Promise.reject(); //Nothing to cimpute or invalid input
                }
            }
        } catch (error) {
    ...
    static validateCharacteristic(fdcControl, section, index, context) {
        ...
        if (libThis.isQuantitative(section.binding)) {
            let valueControl = 'QuantitativeValue' + suffix;
            let value = fdcControl.getCellProxyWithName(valueControl).getValue();
    ...
    static setCharacteristicValuation(suffix, value, section, fdcControl, context) {
        ...
        if (value) { //For the purposes of the filter, only validate if user entered a value
            value = parseFloat(value); // convert from string to float
            let valueAccepted = true;
            if (section.binding.LowerLimitFlag === 'X' && value < section.binding.LowerLimit) {
                valueAccepted = false;
            }
            if (section.binding.UpperLimitFlag === 'X' && value > section.binding.UpperLimit) {
                valueAccepted = false;
            }
            if (valueAccepted) {
